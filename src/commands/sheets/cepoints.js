...
const { staff_company } = require('../../../config.json');
    ...
    switch (n) {
      ...
      case 5:
        try {
          ...
          return sendSuccessEmbed(embed, `Successfully changed ${args[1]}'s CEP by \`${args[3]}\` for reason: \`${reason}\``, message);
        } catch (error) {
          console.log(`cep change: ${error}`);
        }
        break;
    }
  },
