    ...
    switch (n) {
      ...
      case 5:
        try {
          ...
          return sendSuccessEmbed(embed, `Successfully changed ${args[1]}'s EP by \`${args[3]}\` for reason: \`${reason}\``, message);
        } catch (error) {
...
const sendUserEmbed = async (embed, user, message) => {
  embed
    ...
    .addFields(
      ...
      { name: 'Weekly EP', value: user.c[4].v.toString(), inline: true },
      { name: 'Total EP', value: user.c[5].v.toString(), inline: true },
